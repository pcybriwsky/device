...
const myP5Sketch = (p) => {
  ...
  p.setup = async () => {
    ...
    co2Slider = p.createSlider(200, 600, 400);
  ...
  p.draw = () => {
    ...
    if (!isMagic) {
      ...
      p.text("AQI", aqiSlider.x * 2 + aqiSlider.width, 4.5 * lineHeight);
    ...
    p.text("Demo Instnace Initialized: " + initializationDate.toLocaleString(), p.width - 25, p.height - lineHeight);
